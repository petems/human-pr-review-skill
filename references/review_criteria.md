...
Use clear, bracketed tags to indicate severity. Pair the tag with an emoji
internally (in `pr/review.md`) for at-a-glance scanning, but rely on the
bracketed text in `pr/human.md` since GitHub renders it more reliably and the
words still convey severity if the emoji is stripped.
...
| 💡 `[suggestion]`| Consider for the future, no action required this PR|
| 📚 `[learning]` | Educational context. Pure FYI, no action needed    |
| ❓ `[question]` | Clarification needed from the author               |
| 🎉 `[praise]`   | Good work, worth calling out                       |

For the *how to phrase* the feedback (question approach, suggest don't command,
sandwich method, handling disagreements), see
[`human_review_principles.md`](./human_review_principles.md).
